...
class TeamMemberModel with TopModel {
  ...
  set setId(String idParameter) {
    ...
    if (idParameter.isEmpty) {
      exception = Exception(Constants.team_member_id_empty_error_key.tr);
      throw exception;
    }
    id = idParameter;
  }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception =
          Exception(Constants.team_member_creating_time_future_error_key.tr);
      throw exception;
    }
    //تاريخ إضافةالدوكيومنت الخاص بالعضو لا يمكن أن يكون قبل الوقت الحالي
    if (firebaseTime(createdAtParameter).isBefore(now)) {
      exception =
          Exception(Constants.team_member_creating_time_past_error_key.tr);
      throw exception;
    }
    createdAt = createdAtParameter;
  }
  ...
  set setUpdatedAt(DateTime updatedAtParameter) {
    ...
    if (updatedAtParameter.isBefore(createdAt)) {
      exception = Exception(
          Constants.team_member_updating_time_before_create_error_key.tr);
      throw exception;
    }
    updatedAt = updatedAtParameter;
  }
