...
class ProjectModel extends Var2TopModel {
  ...
  set setImageUrl(String imageUrl) {
    ...
    if (imageUrl.isEmpty) {
      ...
      exception = Exception(Constants.project_imageUrl_empty_error_key.tr);
      throw exception;
    }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception =
          Exception(Constants.project_creating_time_future_error_key.tr);
      throw exception;
    }
    //تاريخ إضافة الدوكيومنت الخاص بالمشروع لا يمكن أن يكون قبل الوقت الحالي
    if (createdAtParameter.isBefore(now)) {
      exception = Exception(Constants.project_creating_time_past_error_key.tr);
      throw exception;
    }
    createdAt = createdAtParameter;
  }
  ...
  set setUpdatedAt(DateTime updatedAtParameter) {
    ...
    if (updatedAtParameter.isBefore(createdAt)) {
      exception =
          Exception(Constants.project_updating_before_creating_error_key.tr);
      throw exception;
    }
    updatedAt = updatedAtParameter;
  }
  ...
  set setId(String idParameter) {
    ...
    if (idParameter.isEmpty) {
      exception = Exception(Constants.project_id_empty_error_key.tr);
      throw exception;
    }
    id = idParameter;
  }
  ...
  set setName(String nameParameter) {
    ...
    if (nameParameter.isEmpty) {
      exception = Exception(Constants.project_name_empty_error_key.tr);
      throw exception;
    }
    //لايمكن ان يكون اسم المشروع مؤلف من اقل من ثلاث محارف
    if (nameParameter.length <= 3) {
      exception = Exception(Constants.project_name_length_error_key.tr);
      throw exception;
    }

    //لايمكن ان يحوي اسم المشروع اي ارقام او محارف خاصة فقطط احرف
    if (_regex.hasMatch(nameParameter)) {
      exception = Exception(Constants.project_name_format_error_key.tr);
      throw exception;
    }
  ...
  set setStartDate(DateTime? startDateParameter) {
    ...
    if (startDateParameter == null) {
      exception = Exception(Constants.project_start_date_null_error_key.tr);
      throw exception;
    }
    ...
    if (startDateParameter.isBefore(now)) {
      exception = Exception(Constants.project_start_date_past_error_key.tr);
      throw exception;
    }
  ...
  set setEndDate(DateTime? endDateParameter) {
    ...
    if (endDateParameter == null) {
      exception = Exception(Constants.project_end_date_null_error_key.tr);
      throw exception;
    }
    ...
    if (endDateParameter.isBefore(getStartDate)) {
      exception = Exception(Constants.project_end_time_error_key.tr);
      throw exception;
    }
    //لا يمكن أن يكون تاريخ نهاية المشروع بنفس وقت تاريخ بداية المشروع
    if (getStartDate.isAtSameMomentAs(endDateParameter)) {
      exception = Exception(Constants.project_end_time_same_error_key.tr);
      throw exception;
    }
    ...
    if (diff.inMinutes < 5) {
      exception = Exception(Constants.project_time_difference_error_key.tr);
      throw exception;
    }
    endDate = endDateParameter;
  }
