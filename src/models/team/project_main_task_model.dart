...
class ProjectMainTaskModel extends TaskClass {
  ...
  set setId(String idParameter) {
    ...
    if (idParameter.isEmpty) {
      exception = Exception(Constants.project_main_task_id_empty_key);
      throw exception;
    }
    id = idParameter;
  }
  ...
  set setName(String nameParameter) {
    ...
    if (nameParameter.isEmpty) {
      exception = Exception(Constants.project_main_task_name_empty_key);
      throw exception;
    }
  ...
  set setprojectId(String projectIdParameter) {
    ...
    if (projectIdParameter.isEmpty) {
      exception = Exception(Constants.project_id_empty_key);
      throw exception;
    }
    projectId = projectIdParameter;
  }
  ...
  set setStatusId(String statusIdParameter) {
    ...
    if (statusIdParameter.isEmpty) {
      exception = Exception(Constants.main_task_status_id_empty_key);
      throw exception;
    }
  ...
  set setimportance(int importanceParameter) {
    ...
    if (importanceParameter < 1) {
      exception = Exception(Constants.main_task_importance_min_invalid_key);
      throw exception;
    }
    //لا يمكن أن تكون للأهمية قيمة أكبر من 5
    if (importanceParameter > 5) {
      exception = Exception(Constants.main_task_importance_max_invalid_key);
      throw exception;
    }
    importance = importanceParameter;
  }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception =
          Exception(Constants.main_task_create_time_not_in_future_invalid_key);
      throw exception;
    }
    //تاريخ إضافة الدوكيومنت لا يمكن أن يكون قبل الوقت الحالي تذكر المستخدم غبي المطور أغبى
    if (createdAtParameter.isBefore(now)) {
      exception = Exception(Constants.main_task_create_time_in_past_error_key);
      throw exception;
    }
  ...
  set setUpdatedAt(DateTime updatedAtParameter) {
    ...
    if (updatedAtParameter.isBefore(createdAt)) {
      exception = Exception(
          Constants.main_task_updating_time_not_in_future_invalid_key);
      throw exception;
    }
    updatedAt = (updatedAtParameter);
  }
  ...
  set setStartDate(DateTime? startDateParameter) {
    ...
    if (startDateParameter == null) {
      ...
      exception = Exception(Constants.main_task_start_date_null_key);
      throw exception;
    }
    ...
    if (startDateParameter.isBefore(now)) {
      exception = Exception(Constants.main_task_start_date_past_error_key);
      throw exception;
    }
  ...
  set setEndDate(DateTime? endDateParameter) {
    ...
    if (endDateParameter == null) {
      exception = Exception(Constants.main_task_end_date_null_key);
      throw exception;
    }
    ...
    if (endDateParameter.isBefore(startDate)) {
      exception = Exception(Constants.main_task_start_after_end_error_key);
      throw exception;
    }
    ...
    if (diff.inMinutes < 5) {
      exception = Exception(Constants.main_task_date_difference_error_key);
      throw exception;
    }
    ...
    if (endDateParameter.isAtSameMomentAs(getStartDate)) {
      exception = Exception(
        Constants.main_task_start_same_as_end_error_key,
      );
      throw exception;
    }
