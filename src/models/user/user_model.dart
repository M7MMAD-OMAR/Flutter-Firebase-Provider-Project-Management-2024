...
//الباني
  ...
  set setName(String name) {
    ...
    if (name.isEmpty) {
      ...
      exception = Exception(Constants.name_not_empty_key.tr);
      throw exception;
    }
    if (name.length <= 3) {
      //لايمكن ان يكون الاسم مألف من اقل من ثلاث محارف
      exception = Exception(Constants.name_min_length_key.tr);
      throw exception;
    }
    if (regEx.hasMatch(name) || regEx2.hasMatch(name)) {
      exception = Exception(Constants.name_letters_only_key.tr);
      throw exception;
    }
  ...
  set setUserName(String? userName) {
    ...
    if (userName.isEmpty) {
      ...
      exception = Exception(Constants.username_not_empty_key.tr);
      throw exception;
    }
    if (userName.length < 3) {
      //لايمكن ان يكون اليوزرنيم مألف من اقل من ثلاث محارف
      exception = Exception(Constants.username_min_length_key.tr);
      throw exception;
    }
    if (userName.length >= 20) {
      //لايمكن ان يكون اليوزرنيم مألف من اكثر من ثلاث محارف
      exception = Exception(Constants.username_max_length_key.tr);
      throw exception;
    }
    this.userName = userName;
  }
  ...
  set setImageUrl(String imageUrl) {
    ...
    if (imageUrl.isEmpty) {
      ...
      exception = Exception(Constants.user_imageUrl_empty_key.tr);
      throw exception;
    }
  ...
  set setEmail(String? email) {
    ...
    if (email != null) {
      if (!EmailValidator.validate(email)) {
        ...
        throw Exception(Constants.valid_email_error_key.tr);
      }
      this.email = email;
    } else {
      this.email = email;
...
*/
  ...
  set setId(String id) {
    ...
    if (id.isEmpty) {
      exception = Exception(Constants.user_id_empty_error_key.tr);
      throw exception;
    }
    this.id = id;
  }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception = Exception(Constants.user_creating_time_future_error_key.tr);
      throw exception;
    }
    //تاريخ إضافة اليوزر لا يمكن أن يكون قبل الوقت الحالي
    if (createdAtParameter.isBefore(now)) {
      exception = Exception(Constants.user_creating_time_past_error_key.tr);
      throw exception;
    }
    createdAt = createdAtParameter;
  }
  ...
  set setUpdatedAt(DateTime updatedAtParameter) {
    ...
    if (updatedAtParameter.isBefore(createdAt)) {
      exception = Exception(
          Constants.user_account_updating_time_before_creating_error_key.tr);
      throw exception;
    }
    updatedAt = updatedAtParameter;
  }
