...
class ProjectController extends ProjectAndTaskController {
  ...
  Future<void> updateProject2(
    ...
    if (data.containsKey(startDateK)) {
      ...
      if (newStartDate.isAfter(projectModel!.endDate!)) {
        ...
        Exception exception = Exception(Constants.start_date_update_error_key);
        throw exception;
      }
      if (projectModel.endDate!.isBefore(firebaseTime(DateTime.now()))) {
        Exception exception =
            //"Cannot update the start date after the end of project date has passed"
            Exception(Constants.project_start_date_update_error_key);
        throw exception;
      }
      if (await existByOne(
        ...
        Exception exception =
            Exception(Constants.project_already_started_error_key);
        throw exception;
      }
    }
    if (data.containsKey(teamIdK)) {
      Exception exception = Exception(Constants.team_id_update_error_key);
      throw exception;
    }
    ...
    await updateRelationalFields(
      ...
      nameException: Exception(Constants.already_existing_project_key),
    );
  ...
  Future<void> addProject({required ProjectModel projectModel}) async {
    ...
    if (await existByOne(
      ...
      if (projectModel.teamId != null) {
        ...
        if (await existByTow(
          ...
          if (anotherProjects.docs.isNotEmpty) {
            Exception exception =
                Exception(Constants.team_project_overlap_error_key.tr);
            throw exception;
          } else {
          ...
          Exception exception = Exception(Constants.team_manager_error_key.tr);
          throw exception;
        }
      }
      {
      ...
      Exception exception = Exception(Constants.manager_not_found_error_key.tr);
      throw exception;
    }
  }
...
// لجلب المشروع الخاص بهل الفريق  إن وجد
  ...
  Stream<QuerySnapshot<ProjectModel>> getProjectsOfUserStream(
    ...
    if (managerModel == null) {
      throw Exception(Constants.make_project_first_error_key.tr);
    }
...
//عطيني كل المشاريع يلي مشترك فيها هل الزلمة كعضو
  Stream<QuerySnapshot<ProjectModel?>> getProjectsOfMemberWhereUserIsStream(
    ...
    if (teams.isNotEmpty) {
      ...
      throw Exception(Constants.not_member_no_projects_error_key.tr);
    }
  ...
  Future<void> updateProject(
    ...
    if (data.containsKey(startDateK)) {
      ...
      if (newStartDate.isAfter(oldProject.endDate!)) {
        Exception exception =
            Exception(Constants.start_date_update_error_key.tr);
        throw exception;
      }
      print("update 2");
      ...
      if (oldProject.endDate!.isBefore(firebaseTime(DateTime.now()))) {
        Exception exception =
            Exception(Constants.project_start_date_update_error_key.tr);
        throw exception;
      }
      if (await existByOne(
          collectionReference: projectMainTasksRef,
          ...
          value: id)) {
        Exception exception =
            Exception(Constants.project_already_started_error_key.tr);
        throw exception;
      }
      print("update 3");
    }
    if (data.containsKey(teamIdK)) {
      Exception exception = Exception(Constants.team_id_update_error_key.tr);
      throw exception;
    }
    print("update 3");
    ...
    await updateRelationalFields(
      ...
      fatherField: managerIdK,
      fatherValue: managerModel.id,
      nameException: Exception(Constants.already_existing_project_key.tr),
    );
    //   await updateFields(reference: projectsRef, data: data, id: id);
  }
