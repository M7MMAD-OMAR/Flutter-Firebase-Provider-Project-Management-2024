...
class TeamModel extends VarTopModel {
  ...
  set setImageUrl(String imageUrl) {
    ...
    if (imageUrl.isEmpty) {
      ...
      exception = Exception(Constants.team_image_empty_error_key.tr);
      throw exception;
    }
  ...
  set setId(String id) {
    ...
    if (id.isEmpty) {
      exception = Exception(Constants.team_id_empty_error_key.tr);
      throw exception;
    }
    this.id = id;
  }
  ...
  set setName(String name) {
    ...
    if (name.isEmpty) {
      ...
      exception = Exception(Constants.team_name_empty_error_key.tr);
      throw exception;
    }
    if (name.length < 3) {
      //لايمكن ان يكون اسم الفريق مؤلفاً من اقل من ثلاث محارف
      exception = Exception(Constants.team_name_min_length_error_key.tr);
      throw exception;
    }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception = Exception(Constants.team_creating_time_future_error_key.tr);
      throw exception;
    }
    //تاريخ إضافة الدوكيومنت الخاص بالفريق لا يمكن أن يكون قبل الوقت الحالي
    if (firebaseTime(createdAtParameter).isBefore(now)) {
      exception = Exception(Constants.team_creating_time_past_error_key.tr);
      throw exception;
    }
  ...
  set setUpdatedAt(DateTime updatedAtParameter) {
    ...
    if (updatedAtParameter.isBefore(createdAt)) {
      exception =
          Exception(Constants.team_updating_time_before_creation_error_key.tr);
      throw exception;
    }
    updatedAt = updatedAtParameter;
  }
