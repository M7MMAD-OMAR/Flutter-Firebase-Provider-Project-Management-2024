...
class UserTaskModel extends TaskClass {
  ...
  set setUserId(String userIdParameter) {
    ...
    if (userIdParameter.isEmpty) {
      exception = Exception(Constants.task_user_id_empty_error_key);
      throw exception;
    }
  ...
  set setFolderId(String folderIdParameter) {
    ...
    if (folderIdParameter.isEmpty) {
      exception = Exception(Constants.user_task_category_id_empty_error_key);
      throw exception;
    }
  ...
  set setName(String? nameParameter) {
    ...
    if (nameParameter == null) {
      exception = Exception(Constants.user_task_name_null_error_key);
      throw exception;
    }
    //لا يمكن ان يكون اسم المهمة فارغاً
    if (nameParameter.isEmpty) {
      exception = Exception(Constants.user_task_name_empty_error_key.tr);
      throw exception;
    }
  ...
  set setId(String idParameter) {
    ...
    if (idParameter.isEmpty) {
      exception = Exception(Constants.user_task_id_empty_error_key.tr);
      throw exception;
    }
    id = idParameter;
  }
  ...
  set setStatusId(String statusIdParameter) {
    ...
    if (statusIdParameter.isEmpty) {
      exception = Exception(Constants.task_status_id_empty_error_key.tr);
      throw exception;
    }
    statusId = statusIdParameter;
  }
  ...
  set setImportance(int importanceParameter) {
    ...
    if (importanceParameter <= 0) {
      exception = Exception(Constants.importance_less_than_zero_error_key.tr);
      throw exception;
    }
    //الأهمية لا يمكن أن تكون أكبر من 5 وهي مهم جداً
    if (importanceParameter > 5) {
      exception =
          Exception(Constants.importance_greater_than_five_error_key.tr);
      throw exception;
    }
    importance = importanceParameter;
  }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception = Exception(Constants.user_task_create_future_error_key.tr);
      throw exception;
    }
    //تاريخ إضافة الدوكيومنت لا يمكن أن يكون قبل الوقت الحالي
    if (firebaseTime(createdAtParameter).isBefore(now)) {
      exception = Exception(Constants.user_task_create_past_error_key.tr);
      throw exception;
    }
  ...
  set setUpdatedAt(DateTime updatedAtParameter) {
    ...
    if (updatedAtParameter.isBefore(createdAt)) {
      exception = Exception(Constants.task_update_before_create_error_key.tr);
      throw exception;
    }
  ...
  set setStartDate(DateTime? startDateParameter) {
    ...
    if (startDateParameter == null) {
      ...
      exception = Exception(Constants.user_task_start_date_null_error_key.tr);
      throw exception;
    }
    ...
    if (startDateParameter.isBefore(now)) {
      exception = Exception(Constants.user_task_start_date_past_error_key.tr);
      throw exception;
    }
  ...
  set setEndDate(DateTime? endDateParameter) {
    ...
    if (endDateParameter == null) {
      exception = Exception(Constants.user_task_end_date_null_error_key.tr);
      throw exception;
    }
    ...
    if (endDateParameter.isAtSameMomentAs(startDate)) {
      exception = Exception(
        Constants.user_task_start_end_date_same_time_error_key.tr,
      );
    }
    ...
    if (differeInTime(getStartDate, endDateParameter).inMinutes < 5) {
      exception = Exception(Constants.task_time_difference_error_key.tr);
      throw exception;
    }
    //تاريخ انتهاء المهمة لا يمكن أن يكون قبل بداية المهمة
    if (endDateParameter.isBefore(startDate)) {
      exception = Exception(Constants.user_task_end_date_error_key.tr);
      throw exception;
    }
