...
//? do you want to use this or the second one??
  EitherException<bool> updateEmail({required String email}) async {
    ...
    return Left(Exception(Constants.please_enter_valid_email_key.tr));
  }
  ...
  EitherException<bool> updatePassword({required String newPassword}) async {
    ...
    try {
      ...
      if (e.toString() ==
        ...
        return Left(
            Exception(Constants.sensitive_change_password_process_key.tr));
      }
      return Left(e);
    }
  }
    ...
    try {
      if (user != null) {
        ...
        if (!user.emailVerified) {
          ...
          MySnackBar.showSuccess(Constants.the_email_in_its_way_to_you_key.tr);
          return const Right(false);
        }
        return const Right(true);
      }
      throw Exception(Constants.there_is_no_user_logging_in_or_sign_up_key.tr);
    } on Exception catch (e) {
      dev.log("error");
      return Left(e);
    }
      ...
      switch (e.code) {
        case "provider-already-linked":
          if (kDebugMode) {
            ...
            MySnackBar.showError(
              Constants.The_provider_has_already_been_linked_to_the_user_key.tr,
            );
          }
          break;
        case "invalid-credential":
          if (kDebugMode) {
            ...
            MySnackBar.showError(
              Constants.The_providers_credential_is_not_valid_key.tr,
            );
          }
          break;
